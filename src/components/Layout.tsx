...
export default function Layout({ children, user, onLogout }: LayoutProps) {
  ...
  const isPersuratan = location.pathname.startsWith('/apps/persuratan');

  return (
    <div className="min-h-screen bg-slate-50 flex justify-center">
      {/* Sidebar Navigation */}
      {!isNavHidden && !isPersuratan && (
      ...
      <div className={clsx(
        ...
        (isNavHidden || isPersuratan) ? "max-w-full" : (location.pathname.startsWith('/admin') ? "max-w-full md:ml-64" : "max-w-full md:ml-64 lg:mr-80")
      )}>
        {/* Header */}
        {!isPersuratan && <NotificationBanner />}
        {!isPersuratan && (
      ...
      {!isPersuratan && <RightSidebar user={user} />}
